...
function setupDatabase() {
  ...
  // 🆕 수행 항목 마스터 데이터
  const practiceSheet = ensureSheet(doc, 'PracticeItems', ['id', 'title', 'icon', 'description', 'order']);
  if (practiceSheet.getLastRow() <= 1) {
    practiceSheet.appendRow(['1', '108배', '🙏', '하루 108배 수행', 1]);
    practiceSheet.appendRow(['2', '반야심경', '📿', '반야심경 독송', 2]);
    practiceSheet.appendRow(['3', '천수경', '✨', '천수경 독송', 3]);
    practiceSheet.appendRow(['4', '참선', '🧘', '참선 수행', 4]);
    practiceSheet.appendRow(['5', '염불', '🪷', '염불 수행', 5]);
    practiceSheet.appendRow(['6', '경전 공부', '📖', '경전 읽기', 6]);
    practiceSheet.appendRow(['7', '사경', '✍️', '경전 필사', 7]);
  }
...
function addSchedule(doc, item) {
  ...
  sheet.appendRow([
    ...
    item.location || '', item.maxParticipants || 0, ''
  ]);
  return { message: 'ok' };
}

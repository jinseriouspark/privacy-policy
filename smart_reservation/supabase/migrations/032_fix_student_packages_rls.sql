-- Migration: Fix Student Packages RLS Policies
-- Purpose: Correct RLS policies to properly match auth.uid() with user records
-- Date: 2025-12-26

-- =====================================================
-- FIX STUDENT_PACKAGES RLS POLICIES
-- =====================================================
...
COMMENT ON POLICY "Students can view their own packages" ON student_packages IS 'Allows students to view only their own packages by matching auth.uid() with users.auth_id';
COMMENT ON POLICY "Students can view their own reservations" ON reservations IS 'Allows students to view only their own reservations by matching auth.uid() with users.auth_id';
COMMENT ON POLICY "Students can view their own notifications" ON notifications IS 'Allows students to view only their own notifications by matching auth.uid() with users.auth_id';
